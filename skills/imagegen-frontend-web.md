...
description: Elite frontend image-direction skill for generating premium, conversion-aware website design references. CRITICAL OUTPUT RULE 鈥?generate ONE separate horizontal image FOR EVERY section. A landing page with 8 sections produces 8 images. Never compress multiple sections into one image. Enforces composition variety (not always left-text / right-image), background-image freedom, varied CTAs, varied hero scales (giant / mid / mini minimalist), narrative concept spine, second-read moments, and a single consistent palette across all images. Optimized for landing pages, marketing sites, and product comps that developers or coding models can accurately recreate.
allowed-tools:
  - Read
  - Write
  - Edit
...
# HERO COMPOSITION BIAS 鈥?READ FIRST
...
Use left-text / right-image only when it is genuinely the strongest choice 鈥?not by default.
...
Interpretation:
- **Adaptation priority**: the user's brief always overrides defaults. Read the prompt carefully, then adjust dials, hero scale, background mode, gradient use, and composition variety to match 鈥?never force a recipe that contradicts the brief.
- If the user says "clean", reduce density and increase clarity.
- If the user says "crazy creative", increase variance and art direction.
- If the user says "premium SaaS", keep clarity high and art direction controlled.
- If the user says "editorial", allow stronger type and more asymmetry.
- Bias toward stronger visual concepts, not safe layouts 鈥?but never against the brief.
- Use imagery as a core design material 鈥?including as **full-bleed backgrounds**, not only as inline assets, **when the brief allows it**.
...
- Use defaults from 搂1 + 搂2 with confident background variety
...
The **left-text / right-image** layout is allowed, but it is the most overused AI pattern 鈥?do not use it as the default. Reach for it only when it is the genuinely best fit.
...
- Left-third caption + right-two-thirds visual (classic 鈥?use sparingly, never twice in a row)
...
Pick 1 per section; vary across the page so it is never all the same mode. Be **confident** with backgrounds 鈥?they are a primary tool, not a risk.
...
- Editorial side-image (50/50, 60/40, 40/60 鈥?invertible)
...
### Hero Scale (per-page)
Pick 1 鈥?must match brand mood:
...
Mini does not mean weak 鈥?it means confident restraint.

### Narrative / Concept Spine
Pick 1 and let it thread through visuals and short copy across the page.
- Artifact / collectible 鈥?proof, specimen, treasured object framing
- Journey / pilgrimage 鈥?directional flow, waypoint sections, roadmap feeling
- Tool / precision instrument 鈥?machined detail, calibrated UI, tactile controls
- Living system / garden 鈥?organic growth metaphor, branching layout, nurtured tone
- Stage / spotlight 鈥?theatrical contrast, performer + audience framing
- Archive / dossier 鈥?indexed rows, captions, understated authority
...
- 鈥渢rusted by鈥?ticker that is unreadable mosquito logos
- auto-play-style hero dots with no semantic purpose

### Data / KPI slop
- three identical stat columns (99% satisfaction, $10 saved, 鈭?scale) unless user asked for KPIs
...
Section-level mood shifts must reuse the same palette 鈥?no full theme swap per section.
...
- a tonal gradient,
- a tactile material,
or a confident flat color field 鈥?picked deliberately, not as decoration.
...
Across the slice, deliberately vary foreground/background intensity at least twice (lighter 鈫?richer 鈫?calmer) so the scroll feels paced, not monotonous slabs.
...
Mix at least **two distinct image crops** where multiple sections exist 鈥?e.g. macro product + contextual environment, or portrait editorial + widescreen artifact 鈥?avoiding one repeated stock silhouette.
...
When the brief names an industry or region, steer palette and typographic temperament to match 鈥?don鈥檛 ship default 鈥渘eutral SF startup鈥?unless the brief is intentionally generic SaaS.
...
2. infer number of sections (if unclear, use the defaults from 搂5: landing page = 6, full website = 8)
3. **commit out loud** to the section count and announce it ("Generating N horizontal images, one per section")
4. plan ONE horizontal image PER SECTION 鈥?always separate generations, never collapse
...
7. for each section: pick a Composition Anchor, Background Mode, and CTA Variation 鈥?vary across sections
...
12. apply 搂18 EXTRA CREATIVITY & IMPLEMENTATION EDGE
13. keep spacing generous, even, and clean
14. remove AI slop (including marquee / fake KPI clich茅s unless requested)
15. run 搂17 CLARITY CHECK
